...
TICKERS = ["joby", "aur", "lth", "zm", "naut", "isrg", "ionq", "coin", "anthropic", "rklb", "oklo", "achr", "gral", "txg", "lulu", "abnb", "uber", "yeti", "dash", "ilmn", "shak", "meta", "amzn", "googl", "aapl", "nvda", "tsla", "dis", "cmg", "dal", "hood", "de", "algn", "adsk", "cart", "u", "crox", "rddt", "tost", "wrby", "you", "shop", "rxrx", "beam", "pacb", "tem", "serv", "prme", "twst", "sym", "nvcr", "cai", "hhh"]
...
def build_crypto() -> list:
    """Crypto holdings (data/_crypto.yml) → lightweight portfolio entries.

    Crypto has no cash flows, so each asset is valued with a crypto-native
    framework (spec §16): a scenario-weighted at-maturity target, DISCOUNTED to a
    present value at a crypto-appropriate required return so its upside is
    comparable to the equity DCF book (which already states a today-intrinsic
    value). These are NOT memos — no PDF, no memo page — so they're emitted as a
    separate CRYPTO array. The portfolio page and universe matrix merge them with
    MEMOS; build_categories never sees them, so no home-page category card or
    CategoryPage tries to open a nonexistent memo.

      present_value = target / (1 + discount_rate) ** horizon_years
      expected_fair = Σ probability × present_value;  upside = fair/spot − 1
    """
    f = DATA / "_crypto.yml"
    if not f.exists():
        return []
    ...
    for a in doc.get("assets", []):
        spot = float(a["spot"])
        disc = (1.0 + float(a["discount_rate"])) ** float(a["horizon_years"])
        scen, wpv = [], 0.0
        for key in SCEN_ORDER:
            s = (a.get("scenarios") or {}).get(key)
            if not s:
                continue
            tgt, p = float(s["target"]), float(s["probability"])
            pv = tgt / disc
            wpv += p * pv
            scen.append({
                "key": SITE_KEY[key],
                ...
                "target": tgt,
                "presentValue": round(pv, 2),
                "assumption": collapse(s.get("assumption", "")),
            })
        upside = (wpv / spot - 1.0) if spot > 0 else 0.0
        wl = a["watchlist"]
        out.append({
            "ticker": a["ticker"],
            "slug": a.get("slug", a["ticker"].lower()),
            "company": a["name"],
            "crypto": True,
            "spot": {"price": spot, "asOf": a.get("spot_date", "")},
            "expected": {"fair": round(wpv, 2), "deltaPct": round(upside * 100, 1)},
            "ai": None,
            "taxonomy": {
                "watchlist": wl,
                "watchlistName": TAXONOMY["watchlists"][wl]["name"],
            ...
            "cryptoValuation": {
                "methodShort": collapse(a.get("method_short", "")),
                "method": collapse(a.get("method", "")),
                "horizonYears": float(a["horizon_years"]),
                "discountRate": float(a["discount_rate"]),
                "sources": collapse(a.get("sources", "")),
                "scenarios": scen,
            },
        })
    return out
...
def main() -> None:
    ...
    print(f"wrote {OUT.relative_to(REPO)}  ({len(memos)} memos, {len(crypto)} crypto, {OUT.stat().st_size:,} bytes)")
    for m in memos:
